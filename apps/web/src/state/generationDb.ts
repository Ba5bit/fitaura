...
import type { FullGenerationResult, VersusMode, VersusResult } from '@fitaura/shared';
...
/** A saved Friend-vs-Friend battle (on-device history, like a Solo result). Holds
 * everything the Result deck needs to re-render the verdict without re-running the
 * AI: the two contenders, their photos, and the computed/copy verdict. */
export interface SavedBattle {
  battleId: string;
  /** When this battle was produced (device-local history; the expiry clock). */
  producedAt: string;
  /** Optional user-given name for the vault (defaults to "A vs B"). */
  name?: string;
  mode: VersusMode;
  nameA: string;
  nameB: string;
...
/** One stored battle row. */
interface StoredBattle {
  id: string; // `${accountKey}::${battleId}`
  accountKey: string;
  battle: SavedBattle;
}
...
// --- Friend-vs-Friend battles (same on-device DB, separate `battles` store) ---

const battleRowId = (accountKey: string, battleId: string) => `${accountKey}::${battleId}`;

const newestBattle = (a: SavedBattle, b: SavedBattle) =>
  Date.parse(b.producedAt) - Date.parse(a.producedAt);

/** Newest `cap` battles by produced time. */
export function trimBattlesToCap(battles: SavedBattle[], cap = SAFETY_CAP): SavedBattle[] {
  return [...battles].sort(newestBattle).slice(0, cap);
}

async function getBattles(d: IDBDatabase, accountKey: string): Promise<SavedBattle[]> {
  const idx = d.transaction(BATTLES, 'readonly').objectStore(BATTLES).index('by_account');
  const rows = (await reqDone(idx.getAll(accountKey))) as StoredBattle[];
  return rows.map((r) => r.battle);
}

export async function putBattle(accountKey: string, battle: SavedBattle): Promise<void> {
  const d = await db();
  const tx = d.transaction(BATTLES, 'readwrite');
  tx.objectStore(BATTLES).put({ id: battleRowId(accountKey, battle.battleId), accountKey, battle });
  await txDone(tx);
  // Enforce the safety cap (separate tx so the write above has committed). Single
  // serialized writer (the UX flow), so this read-after-write is not concurrency-safe by design.
  const all = await getBattles(d, accountKey);
  if (all.length > SAFETY_CAP) {
    const keep = new Set(trimBattlesToCap(all).map((b) => b.battleId));
    const tx2 = d.transaction(BATTLES, 'readwrite');
    const store = tx2.objectStore(BATTLES);
    for (const b of all) {
      if (!keep.has(b.battleId)) store.delete(battleRowId(accountKey, b.battleId));
    }
    await txDone(tx2);
  }
}

export async function deleteBattle(accountKey: string, battleId: string): Promise<void> {
  const d = await db();
  const tx = d.transaction(BATTLES, 'readwrite');
  tx.objectStore(BATTLES).delete(battleRowId(accountKey, battleId));
  await txDone(tx);
}

export async function renameBattleDb(accountKey: string, battleId: string, name: string): Promise<void> {
  const d = await db();
  const id = battleRowId(accountKey, battleId);
  const row = (await reqDone(d.transaction(BATTLES, 'readonly').objectStore(BATTLES).get(id))) as StoredBattle | undefined;
  if (!row) return;
  const next: StoredBattle = { ...row, battle: { ...row.battle, name: name.trim() || undefined } };
  const tx = d.transaction(BATTLES, 'readwrite');
  tx.objectStore(BATTLES).put(next);
  await txDone(tx);
}
