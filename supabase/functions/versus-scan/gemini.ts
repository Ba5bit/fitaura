// supabase/functions/versus-scan/gemini.ts
...
/** One labelled inline image: the label is emitted as a text part right before
 * the image part so the model knows which contender/modality it is looking at. */
...
export interface GeminiOpts {
  apiKey: string;
  model: string;
  /** The active set of labelled images (up to 4 for a both-mode battle). */
  images: LabeledImage[];
  /** System instruction (the versus prompt). */
  systemInstruction: string;
  /** OpenAPI-subset structured-output schema (the versus response schema). */
...
function buildBody(opts: GeminiOpts) {
  ...
  for (const { label, image } of opts.images) {
